...
Description
SlotSync is a full-stack scheduling and booking application inspired by Cal.com. It allows users to register, authenticate, create event types, define availability, and share public booking links where others can schedule meetings.
Live Demo
...
Local Setup
1. Clone Repository
bashgit clone https://github.com/Vivekreddy1201/scaler-cal.com-clone
...
Removed the "single default user / no auth" assumption and replaced it with the JWT auth model
Added an Authentication section under Core Features describing the token flow
Added auth endpoints (/api/auth/register, /login, /refresh, /logout) to the API reference
Marked dashboard/management endpoints as protected
Added sample API calls for login and token refresh
Added an Environment Variables section for JWT config
Updated the backend description to mention JWT middleware
